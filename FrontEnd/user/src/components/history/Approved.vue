    ...
    <el-card>
      <el-table :data="applicationList" stripe>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <SignDialog :data="scope.row" ></SignDialog>
            <Upload :data="scope.row"></Upload>
            <FileList  :data="scope.row"></FileList>
            <a type="primary" @click="handleLeave(scope.row)" size="mini">请假信息</a>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    handleLeave(data){
      console.log('请假')
      console.log(this)
      this.$router.push({ path: `/leave/${data.applicationId}` })
    }
  },
};
</script>
