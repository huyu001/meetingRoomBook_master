    ...
    <el-card>
      <div class="resRoomPluginBox">
        <div class="resHead">
          <div class="identifyLst">
            <li
              class="identify"
              v-for="(item, index) in identifyLst"
              :key="index"
            >
              <i :class="{ nooccupy: index == 1, select: index == 2 }"></i
              ><span>{{ item }}</span>
            </li>
          </div>
        </div>
            ...
            <ul class="roomGrid">
              <li
                ...
                :class="{
                  isOccupy: room == false,
                  isSelected:
                    item.roomId == selectedInfo.roomId &&
                    index == selectedInfo.applySlot - 1,
                }"
                @click="
                  selectRoom(
                    item.roomNo,
      ...
      >
        ...
        <el-form-item label="会议室设备：" size="mini">
          <div class="tag">
            <el-tag type="success" v-if="selectedInfo.air === true"
              >空调可用</el-tag
            >
            <el-tag type="danger" v-else-if="selectedInfo.air === false"
              >空调不可用</el-tag
            >
            <el-tag type="info" v-else>空调状态</el-tag>
          </div>
          <div class="tag">
            <el-tag type="success" v-if="selectedInfo.projector === true"
              >投影仪可用</el-tag
            >
            <el-tag type="danger" v-else-if="selectedInfo.projector === false"
              >投影仪不可用</el-tag
            >
        ...
        <el-form-item label="会议主题：" prop="meetingTheme">
          <el-input
            ...
            v-model="selectedInfo.meetingTheme"
          ></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    async getRoomList() {
      ...
      const { data: res2 } = await this.$http.get("/export/mysql", { params });
        console.log(1)
              console.log(res2);
      const { data: res } = await this.$http.get("/apply/free", {
